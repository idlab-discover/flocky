...
// type MemoryChannel interface {
// 	Cleanup()
// 	Init(go2pyName string, py2goName string)
// 	StartRead()
// 	Write()
// 	ReadSingle()
// }

// type ReadOnceMemoryChannel struct {
...
/*func (channel *ReadOnceMemoryChannel) Init(shmFilePath string, semaphore string, name string, expectedSize int64, create bool) error {
	channel.semaphoreName = semaphore + name
	channel.Semaphore = createOrAttachToSemaphore(channel.semaphoreName)
	if channel.Semaphore == nil {
		log.Fatalf("Failed to create semaphore readonce %s", channel.semaphoreName)
	}

	channel.ShmFilePath = shmFilePath
	channel.Name = name

	path := fmt.Sprintf("%s%s", shmFilePath, name)
	var err error
	if create {
...
func (channel *ReadWriteMemoryChannel) Init(shmFilePath string, semWaitName string, semSignalName string, id string, expectedSize int64, create bool) error {
	...
	if channel.SemWait == nil {
		log.Fatalf("Failed to create semaphore go2py %s", channel.semWaitName)
	}
	channel.SemPost = createOrAttachToSemaphore(channel.semPostName)
	if channel.SemPost == nil {
		log.Fatalf("Failed to create semaphore py2go %s", channel.semPostName)
	}
...
func (channel *ReadWriteMemoryChannel) StartWrite(writer func([]byte) error) {
	go func() {
		for {
			// Move defer outside the loop to prevent stack growth
			err := writer(channel.Data)
			if err != nil {
				log.Error(err.Error())
			}
